...
def compare_headers(txt_file, excel_file):
    try:
        ...
        df_excel = pd.read_excel(excel_data, sheet_name=1)
...
txt_file = st.file_uploader("📂 Wybierz plik TXT", type=["txt"], key="txt_upload")
excel_file = st.file_uploader("📂 Wybierz plik Excel", type=["xlsx"], key="excel_upload")

if txt_file and excel_file:
    try:
        # Logowanie dla debugowania
        st.write(f"✅ Załadowano pliki: {txt_file.name}, {excel_file.name}")
